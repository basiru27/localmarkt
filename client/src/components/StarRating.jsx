      ...
      >
        {[1, 2, 3, 4, 5].map((star) => {
          ...
          return (
            ...
            >
              <svg
                className={`${starSize} ${isFilled ? 'text-[#E8A838]' : isPartial ? 'text-[#E8A838]' : 'text-gray-300'} transition-colors`}
...
export function StarRatingCompact({ rating, reviewCount, size = 'sm' }) {
  ...
  return (
    <div className={`inline-flex items-center gap-1 ${textSizeClasses[size]}`}>
      <svg
        className={`${sizeClasses[size]} text-[#E8A838]`}
        fill="currentColor"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
      ...
      <span className="text-[#6B6B6B] text-xs ml-0.5">({reviewCount})</span>
    </div>
  );
}
